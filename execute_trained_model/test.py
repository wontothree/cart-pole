...
try:
    ...
    print("Q-테이블이 성공적으로 로드되었습니다.")
except Exception as e:
    print(f"Q-테이블 로드 중 오류 발생: {e}")
    exit()

def get_action(observation):
    # 상태를 이산화하여 인덱스를 찾는 방식 필요
    state_index = int(observation[0] * 10) % q_table.shape[0]  # 간단한 예시
    action = np.argmax(q_table[state_index])
    return int(np.clip(action, 0, 1))  # 0 또는 1로 제한
