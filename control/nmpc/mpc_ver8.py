...
class CartPoleNMPC:
    def __init__(self):
        ...
        self.states = casadi.SX.sym("states", self.nx)
        self.ctrls = casadi.SX.sym("ctrls", self.nu)
    ...
    def calc_continuous_dynamics(self):
        ...
        f = casadi.Function("f", [self.states, self.ctrls], [states_dot], ['x', 'u'], ['x_dot'])
    ...
    def calc_discrete_dynamics(self):
        ...
        r1 = f(x = self.states, u = self.ctrls)["x_dot"]
        r2 = f(x = self.states + self.dt * r1 / 2, u = self.ctrls)["x_dot"]
        r3 = f(x = self.states + self.dt * r2 / 2, u = self.ctrls)["x_dot"]
        r4 = f(x = self.states + self.dt * r3, u = self.ctrls)["x_dot"]

        states_next = self.states + self.dt * (r1 + 2 * r2 + 2 * r3 + r4) / 6

        F_RK4 = casadi.Function("F_RK4", [self.states, self.ctrls], [states_next], ["x", "u"], ["x_next"])
    ...
    def update_next_state(self):
        ...
        f = self.calc_continuous_dynamics()
        ode = f(x = self.states, u = self.ctrls)["x_dot"]

        dae = {"x": self.states, "p": self.ctrls, "ode": ode}
